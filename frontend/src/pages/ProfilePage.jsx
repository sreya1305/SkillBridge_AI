  ...
  return <main className="relative isolate min-h-screen bg-ink text-white pt-20 pb-16 overflow-hidden">
    ...
    <div className="wrap py-12 sm:py-16"><div className="mx-auto max-w-4xl"><div className="mb-10"><p className="eyebrow text-mint">Step 1 of 4 - Build your profile</p><h1 className="mt-4 text-3xl font-extrabold tracking-[-.04em] text-white sm:text-4xl">Tell us about the experience you bring.</h1><p className="mt-4 max-w-2xl leading-7 text-slate-300">Start manually, or upload a resume to keep a reference handy. We will not parse or send any file in this version.</p></div>
      <form onSubmit={(event) => { event.preventDefault(); handleContinue() }} className="space-y-6">
        ...
        <section className="rounded-2xl border border-white/10 bg-[#0e1a34] p-6 sm:p-8 shadow-xl"><h2 className="text-xl font-extrabold text-white">04 Target role</h2><p className="mt-1 text-sm text-slate-400">Choose the role you want to compare against your current skills.</p><div className="mt-6"><select value={selectedRoleId} onChange={(e) => { setSelectedRoleId(e.target.value); selectTargetRole(e.target.value) }} className={`${inputClass} w-full bg-slate-900 text-white`}><option value="">Select a role</option>{allRoles.map((role) => <option key={role.id} value={role.id}>{role.title}</option>)}</select></div><div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-slate-300"><p className="font-semibold text-white">Selected role</p><p className="mt-2 text-sm">{selectedRoleId ? allRoles.find((r) => r.id === selectedRoleId)?.title : 'No role selected'}</p><a href="/target-role" className="mt-3 inline-block text-sm font-bold text-mint hover:text-mint/80 transition-colors">Add a custom target role</a></div>{!selectedRoleId && <p className="mt-2 text-xs font-semibold text-red-400">Please select a target role before continuing.</p>}</section>
