  ...
  return <main className="relative isolate min-h-screen bg-ink text-white pt-20 pb-16 overflow-hidden">
    ...
    <div className="wrap py-12 sm:py-16">
      ...
      <form onSubmit={(event) => { event.preventDefault(); handleContinue() }} className="space-y-6">
        ...
        <section className="rounded-2xl border border-dashed border-violet-500/40 bg-violet-950/20 p-6 sm:p-8"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center"><div><p className="font-bold text-white">Or upload your resume file / picture</p><p className="mt-1 text-sm text-slate-300">PDF, DOCX, PNG, JPG, WEBP (up to 10 MB). AI & OCR will automatically extract your skills, education, and experience!</p></div><button type="button" onClick={() => { if (typeof window !== 'undefined') { window.localStorage.setItem('userSkills', JSON.stringify(skills.map(s => s.name))); window.localStorage.setItem('userEducation', JSON.stringify(education)); window.localStorage.setItem('userExperience', JSON.stringify(experience)) } window.location.href = '/resume-parser' }} className="rounded-xl border border-violet bg-violet/20 px-6 py-3.5 text-sm font-bold text-violet-200 hover:bg-violet/30 transition-all shadow-glow shrink-0">Upload & Parse Resume →</button></div></section>
