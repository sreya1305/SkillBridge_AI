...
export default function ProfilePage() {
  ...
  const [selectedRoleId, setSelectedRoleId] = useState(roles.roles[0]?.id || '')
  ...
  return <main className="min-h-screen bg-[#f7f8fa] text-ink">
    ...
    <div className="wrap py-12 sm:py-16"><div className="mx-auto max-w-4xl"><div className="mb-10"><p className="eyebrow text-violet">Step 1 of 4 - Build your profile</p><h1 className="mt-4 text-3xl font-extrabold tracking-[-.04em] sm:text-4xl">Tell us about the experience you bring.</h1><p className="mt-4 max-w-2xl leading-7 text-slate-600">Start manually, or upload a resume to keep a reference handy. We will not parse or send any file in this version.</p></div>
      <form onSubmit={(event) => { event.preventDefault(); setNotice('Your profile is ready for the next step. Nothing has been sent or saved yet.') }} className="space-y-6">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8"><h2 className="text-xl font-extrabold">02 Target role</h2><p className="mt-1 text-sm text-slate-500">Choose the role you want to compare against your current skills.</p><div className="mt-6"><select value={selectedRoleId} onChange={(e) => setSelectedRoleId(e.target.value)} className={`${inputClass} w-full bg-white`}><option value="">Select a role</option>{roles.roles.map((role) => <option key={role.id} value={role.id}>{role.title}</option>)}</select></div><div className="mt-6 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600"><p className="font-semibold">Selected role</p><p className="mt-2 text-sm">{selectedRole.title}</p></div></section>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8"><h2 className="text-xl font-extrabold">03 Skill gap analysis</h2><p className="mt-1 text-sm text-slate-500">Compare your current skills with the selected role requirements.</p><div className="mt-6 grid gap-4 lg:grid-cols-3">
            {['critical', 'important', 'niceToHave'].map((category) => {
              const label = category === 'critical' ? 'Critical' : category === 'important' ? 'Important' : 'Nice to have'
              const missing = skillGap.missing[category]
              const matched = skillGap.matched[category]
              return <div key={category} className="rounded-2xl border border-slate-200 bg-[#f9f8ff] p-4">
                <div className="flex items-center justify-between gap-3">
                  <div><p className="text-sm font-semibold">{label}</p><p className="text-xs uppercase tracking-[.22em] text-slate-500">{matched.length}/{matched.length + missing.length} matched</p></div>
                  <span className={`rounded-full px-3 py-1 text-xs font-bold ${category === 'critical' ? 'bg-red-100 text-red-700' : category === 'important' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-700'}`}>{label}</span>
                </div>
                <div className="mt-4 space-y-2 text-sm">
                  {missing.length ? missing.map((skillItem) => <div key={skillItem} className="rounded-xl bg-white px-3 py-2 text-slate-700">{skillItem}</div>) : <div className="rounded-xl bg-white px-3 py-2 text-slate-700">No gaps in this category.</div>}
                </div>
              </div>
            })}
          </div></section>
        {notice && <p role="status" className="rounded-xl bg-mint/50 px-4 py-3 text-sm font-semibold">{notice}</p>}<div className="flex flex-col-reverse justify-between gap-4 pt-2 sm:flex-row sm:items-center"><p className="text-sm text-slate-500">You can update this information later.</p><button type="submit" className="rounded-full bg-ink px-6 py-3 text-sm font-bold text-white">Continue to target role -&gt;</button></div>
      </form></div></div>
  </main>
}
